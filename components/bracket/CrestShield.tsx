...
const ANIMAL_CHARGE_EMOJIS: Record<string, string> = {
  cheetah: "🐆",
  cobra: "🐍",
  elephant: "🐘",
  falcon: "🦅",
...
function renderAnimalEmojiCharge(charge: string): ReactElement | null {
  const emoji = ANIMAL_CHARGE_EMOJIS[charge];
...
function renderCharge(crest: TeamCrest): ReactElement | null {
  ...
  const charge = renderAnimalEmojiCharge(crest.charge) ?? (() => {
    switch (crest.charge) {
      case "cross":
        return (
